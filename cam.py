...
def main():
    ...
    assert os.environ.get("WYZE_EMAIL"), "missing WYZE_EMAIL"
    assert os.environ.get("WYZE_PASSWORD"), "missing WYZE_PASSWORD"
    auth_info = login(os.environ.get("WYZE_EMAIL"), os.environ.get("WYZE_PASSWORD"))
    account = get_user_info(auth_info)
    cameras = get_camera_list(auth_info)

    cam = [camera for camera in cameras if camera.nickname == "Front Porch Cam"][
        0
    ]
